...
frappe.ui.form.on('Purchase Receipt', {
	refresh: function(frm) {
		...
		frm.set_query("kek_transaction", function() {
			...
			return {
				filters: {
					...
					"erpnext_reference_name": parent_po || ""
				}
			};
		});
		...
		// Auto-populate from parent PO if draft and empty
		if (frm.doc.docstatus === 0 && !frm.doc.nomor_ppkek) {
			...
			if (parent_po) {
				frappe.db.get_value("Purchase Order", parent_po, ["nomor_ppkek", "tanggal_ppkek", "kek_status"], (r) => {
					if (r && r.nomor_ppkek) {
